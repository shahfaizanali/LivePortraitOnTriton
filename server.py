...
class StreamHandler:
    ...
    def setup_pipes(self):
        ...
        for pipe in [self.video_pipe, self.audio_pipe]:
            if os.path.exists(pipe):
                os.remove(pipe)
            os.mkfifo(pipe)
        # Open pipes for writing in binary mode
    ...
    def start_ffmpeg(self):
        ...
        cmd = [
            ...
            "rgb24",
            "-s",
            f"{self.video_resolution[0]}x{self.video_resolution[1]}",  # 512x512
            "-r",
            str(self.framerate),  # 15 fps
            "-i",
            self.video_pipe,
            # Audio input
            "-f",
            ...
            "-c:v",
            "h264_nvenc",  # Use NVIDIA's H.264 encoder
            "-preset",
            "p2",  # Preset for lower latency; options range from p1 (slowest) to p7 (fastest)
            "-b:v",
            "2M",  # Video bitrate
            "-maxrate",
            "2M",
            ...
            "4M",
            "-g",
            str(int(self.framerate) * 2),  # GOP size (e.g., 15 fps * 2 = 30)
            "-tune",
        ...
        logger.info(f"Starting FFmpeg for user {self.user_id} with command: {' '.join(cmd)}")
    ...
    async def log_ffmpeg_output(self):
        ...
        while True:
            ...
            if not line:
                break
            logger.info(f"FFmpeg ({self.user_id}): {line.decode().strip()}")
    ...
    def write_video_frame(self, frame: np.ndarray):
        ...
        if self.video_fd:
            try:
                ...
                logger.warning(
                    f"FFmpeg not ready to read video for user {self.user_id}"
                )
            except Exception as e:
                logger.error(
                    f"Error writing video frame for user {self.user_id}: {e}"
                )
    ...
    def write_audio_frame(self, pcm_data: np.ndarray):
        ...
        if self.audio_fd:
            try:
                ...
                logger.warning(
                    f"FFmpeg not ready to read audio for user {self.user_id}"
                )
            except Exception as e:
                logger.error(
                    f"Error writing audio frame for user {self.user_id}: {e}"
                )
    ...
    def stop_ffmpeg(self):
        ...
        if self.ffmpeg_process:
            ...
            self.ffmpeg_process = None
            logger.info(f"FFmpeg process for user {self.user_id} terminated.")
        ...
        for pipe in [self.video_pipe, self.audio_pipe]:
            try:
                ...
                logger.error(f"Error removing pipe {pipe}: {e}")
